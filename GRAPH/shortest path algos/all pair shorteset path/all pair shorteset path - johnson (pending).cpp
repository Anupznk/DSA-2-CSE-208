...
bool bellman_ford(Graph graph, vector <double> &len, int s, int d){
    // O(|v| |E|)
...
int main(){
    ...
    vector <vector<double> > wt2 (v, vector<double>());
    ...
    f.close();
    vector <double> len(v, INF);
    if (bellman_ford(graph, len, sou, dest) == false){
        cout << "Negative cycle";
        exit(1);
    }
